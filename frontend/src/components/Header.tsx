    ...
    <header className={`sticky top-0 z-50 transition-all duration-300 ${
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="flex items-center justify-between py-4">
          ...
          {/* Navigation - Desktop với hiệu ứng liquid fill từ index.html */}
          <nav className="hidden lg:flex items-center space-x-1">
            {navigationItems.map((item, index) => (
              <div key={item.name} className="relative group">
                <a
                  href={item.href}
                  className={`relative px-6 py-3 text-sm font-medium transition-all duration-500 flex items-center space-x-2 rounded-lg overflow-hidden ${
                    isDarkMode 
                ...
                >
                  {/* Liquid fill background effect từ index.html */}
                  <div className="absolute inset-0 bg-gradient-to-r from-teal-500 to-green-400 transform scale-x-0 origin-left transition-transform duration-500 ease-out group-hover:scale-x-100 rounded-lg"></div>
                  ...
                  {item.hasDropdown && (
                    <span className="relative z-10 transform group-hover:rotate-180 transition-transform duration-300 text-xs group-hover:text-white">
                      ▼
                    </span>
                  )}
                </a>
          ...
          <div className="flex items-center space-x-2">
            {/* Search với bo góc hoàn mỹ */}
            <div className="relative">
              <button
                onClick={() => setIsSearchOpen(!isSearchOpen)}
                className={`p-3 rounded-full transition-all duration-300 hover:bg-teal-500/10 ${
                  isDarkMode 
              ...
              {isSearchOpen && (
                <div className={`absolute right-0 top-full mt-2 w-80 rounded-2xl shadow-2xl border p-4 animate-in slide-in-from-top-2 duration-300 ${
                  isDarkMode 
                    ? 'bg-gray-800/95 border-gray-700 backdrop-blur-xl' 
                    : 'bg-white/95 border-gray-200 backdrop-blur-xl'
                }`}>
                  <input
                    ...
                    className={`w-full p-4 rounded-2xl border focus:outline-none focus:ring-2 focus:ring-teal-500 transition-all ${
                      isDarkMode 
            ...
            {/* Notifications */}
            <button className={`relative p-3 rounded-full transition-all duration-300 hover:bg-teal-500/10 ${
              isDarkMode 
                ? 'text-gray-400 hover:text-white' 
                : 'text-gray-600 hover:text-gray-900'
            }`}>
            ...
            <button
              ...
              className={`p-3 rounded-full transition-all duration-300 hover:rotate-180 ${
                isDarkMode 
            ...
            <nav className="space-y-2">
              {navigationItems.map((item) => (
                <a
                  key={item.name}
                  href={item.href}
                  className={`flex items-center justify-between p-3 rounded-xl transition-all duration-300 hover:bg-gradient-to-r hover:from-teal-500/10 hover:to-green-400/10 ${
                    isDarkMode 
                      ? 'text-gray-300 hover:text-white' 
                      : 'text-gray-700 hover:text-gray-900'
                  }`}
